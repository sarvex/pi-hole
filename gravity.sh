...
parseList() {
  ...
  # 1. Add all valid domains to the temporary file and count them
  ...
  if  grep -E "^${abp_domain_pattern}$" -m 1 -q "${src}"; then
    ...
    # count the number of ABP style domains
    num_abp_domains=$(grep -c ^ "${temp_file}")
    num_abp_domains=$((num_abp_domains-num_domains))
  fi
  ...
  if [[ "${num_abp_domains}" -ne 0 ]]; then
    echo -e "${OVER}  ${INFO} Imported ${num_domains} exact and ${num_abp_domains} ABP-style domains"
  else
    echo -e "${OVER}  ${INFO} Imported ${num_domains} exact domains"
  fi
